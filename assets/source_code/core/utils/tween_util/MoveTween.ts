...
export class MoveTween extends Component {
  ...
  @property({
    ...
    tooltip:
      "Choose between specifying positions or selecting nodes for start/end positions.",
  })
  ...
  play(callback?: Function): void {
    ...
    this.currentTween = tween(this.node)
        .to(this.duration, { position: start }, { easing: easingFunction })
        .to(this.duration, { position: end }, { easing: easingFunction });
